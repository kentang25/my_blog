...
class M_datatables extends CI_Model{    
    ...
    function get_tables_query_csrf_groupby($query,$cari,$where,$isWhere,$csrf_name,$csrf_hash,$group_by)
    {
        // Ambil data yang di ketik user pada textbox pencarian
        $search = htmlspecialchars($_POST['search']['value']);
        // Ambil data limit per page
        ...
        if($where != null)
        {
            ...
            if(!empty($isWhere))
            {
                $sql = $this->db->query($query." WHERE $isWhere ".$fwhere. " GROUP BY " . $group_by);
            }else{
                $sql = $this->db->query($query." WHERE ".$fwhere. " GROUP BY " . $group_by);
            }
            $sql_count = $sql->num_rows();
            ...
            if(!empty($isWhere))
            {
                $sql_data = $this->db->query($query." WHERE $isWhere ".$fwhere." AND (".$cari.")". " GROUP BY " . $group_by .$order." LIMIT ".$limit." OFFSET ".$start);
            }else{
                $sql_data = $this->db->query($query." WHERE ".$fwhere." AND (".$cari.")". " GROUP BY " . $group_by .$order." LIMIT ".$limit." OFFSET ".$start );
            ...
            if(isset($search))
            {
                if(!empty($isWhere))
                {
                    $sql_cari =  $this->db->query($query." WHERE $isWhere ".$fwhere." AND (".$cari.")". " GROUP BY " . $group_by);
                }else{
                    $sql_cari =  $this->db->query($query." WHERE ".$fwhere." AND (".$cari.")". " GROUP BY " . $group_by);
                }
                $sql_filter_count = $sql_cari->num_rows();
            }else{
                if(!empty($isWhere))
                {
                    $sql_filter = $this->db->query($query." WHERE $isWhere ".$fwhere. " GROUP BY " . $group_by);
                }else{
                    $sql_filter = $this->db->query($query." WHERE ".$fwhere. " GROUP BY " . $group_by);
                }
                $sql_filter_count = $sql_filter->num_rows();
            }
            ...
            if(!empty($isWhere))
            {
                $sql = $this->db->query($query." WHERE $isWhere". " GROUP BY " . $group_by);
            }else{
                $sql = $this->db->query($query. " GROUP BY " . $group_by);
            }
            ...
            if(!empty($isWhere))
            {
                $sql_data = $this->db->query($query." WHERE $isWhere AND (".$cari.")". " GROUP BY " . $group_by .$order." LIMIT ".$limit." OFFSET ".$start);
            }else{
                $sql_data = $this->db->query($query." WHERE (".$cari.")". " GROUP BY " . $group_by .$order." LIMIT ".$limit." OFFSET ".$start);
            }
            ...
            if(isset($search))
            {
                if(!empty($isWhere))
                {
                    $sql_cari =  $this->db->query($query." WHERE $isWhere AND (".$cari.")". " GROUP BY " . $group_by);
                }else{
                    $sql_cari =  $this->db->query($query." WHERE (".$cari.")". " GROUP BY " . $group_by);
                }
                $sql_filter_count = $sql_cari->num_rows();
            }else{
                ...
                if(!empty($isWhere))
                {
                    $sql_filter =  $this->db->query($query." WHERE $isWhere". " GROUP BY " . $group_by);
                }else{
                    $sql_filter = $this->db->query($query. " GROUP BY " . $group_by);
                }
