...
<script>
    let url = "<?= base_url('admin/master/unit/get_data');?>";
    var table = null;
    ...
    $(document).ready(function() {
        ...
        table = $('table').DataTable({
            ...
            columns: [
                ...
                { data: "id_unit",
                  sortable: false,
                  render: function( data, type, row, meta ) {
	                	// let str =`<a href="<?= base_url().'admin/master/unit/subunit/' ?>${data}" title="subunit" class="btn btn-sm btn-icon btn-warning" ><i class="fa fa-building"></i></a>`;
                    // str += ` `;
                    let str =`<a href="#" title="edit" class="btn btn-sm btn-icon btn-primary" id="editData" data-row='${JSON.stringify(row)}'><i class="far fa-edit"></i></a>`;
	                	str += ` `;
	                	str += `<a href="#" title="hapus" class="btn btn-sm btn-icon btn-danger" id="hapusData" data-id='${data}'><i class="fas fa-trash"></i></a>`;
	                	return str;
	                }
                },
            ],
        ...
        $('#formAdd').submit(function (e) {
          ...
          formData.push({ name: '<?= $this->security->get_csrf_token_name(); ?>', value: token });
            $.ajax({
                  url: "<?= base_url('admin/master/unit/save')?>",
                  type: "POST",
                  dataType:'json',
                  data:formData,
        ...
        $('table').on('click', '#editData', function (e) {
          ...
          // console.log(row)
          $("#formEdit input[name=id_unit]").val(row.id_unit);
          $("#formEdit input[name=nama_unit]").val(row.nama_unit);
        ...
        $('#formEdit').submit(function (e) {
          ...
          formData.push({ name: '<?= $this->security->get_csrf_token_name(); ?>', value: token });
            $.ajax({
                  url: "<?= base_url('admin/master/unit/update')?>",
                  type: "POST",
                  dataType:'json',
                  data:formData,
                  success: function(json){
        ...
        $('table').on('click', '#hapusData', function (e) {
          ...
          .then((willDelete) => {
            if (willDelete) {
              $.ajax({
                  url: "<?= base_url('admin/master/unit/destroy')?>",
                  type: "POST",
                  dataType:'json',
        ...
        $("#massDelete").on("click",function(){
            ...
            .then((willDelete) => {
              if (willDelete) {
                $.ajax({
                    url: "<?= base_url('admin/master/unit/mass_delete')?>",
                    type: "POST",
                    dataType:'json',
