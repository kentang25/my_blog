 ...
 *
 * @author Abdul Haris <harizu.sekai@gmail.com>
 * @copyright Januari 2015
 */
class Dbpdo {
    ...
    public function __construct()
    {
        ...
        // $this->pdo = new PDO("mysql:host="."localhost".";dbname="."bkd_iain", "developer", "!@!nK3diri!" );
    ...
    */
    ...
    public function replace($table,$dat) {
        ...
        $im=implode(', ', $mark);
        $ins = $this->pdo->prepare("REPLACE INTO $table ($col) values ($im)");
        $ins->execute( $data );
    ...
    */
    ...
    public function terpilih($nav,$group_id,$aktif)
    {
      $pilih="";
    ...
    function buildMenu($url, $parent, $menu, $aktif)
    {
        if(!empty($menu)){
            $html = "";
               if (isset($menu['parents'][$parent]))
               {
                   foreach ($menu['parents'][$parent] as $itemId)
                   {
                        ...
                        // var_dump($aktif);
                        if($menu['items'][$itemId]['title'] == "Y"){
                            $html .= '<li class="menu-header">'.$menu['items'][$itemId]['page_name'].'</li>';
                        }else{
                            if(!isset($menu['parents'][$itemId]))
                              {
                                 $html .= "<li ";
                                 // $html .=($url==$menu['items'][$itemId]['url'])?'class="active"':'';
                                 $html .=(in_array($menu['items'][$itemId]['id'],$aktif))?'class="active"':'';
                                 $html.=">
                                   <a class='nav-link' href='".base_url().$menu['items'][$itemId]['url']."'>";
                                 if(!empty($menu['items'][$itemId]['icon']))
                                  {
                                    $html.="<i class='".$menu['items'][$itemId]['icon']."'></i>";
                                  } else {
                              ...
                              if(isset($menu['parents'][$itemId]))
                              {
                                ...
                                $html .= "<li class='dropdown ".$this->terpilih($url,$menu['items'][$itemId]['id'],$aktif);

                                $html.="'><a href='#' class='nav-link has-dropdown'>";
                                 if(!empty($menu['items'][$itemId]['icon']))
                                  {
                                    $html.="<i class='".$menu['items'][$itemId]['icon']."'></i>";
                                  } else {
                                ...
                                $html .=$this->buildMenu($url,$itemId, $menu,$aktif);
                                $html .= "</ul></li>";
                              }
                        }
