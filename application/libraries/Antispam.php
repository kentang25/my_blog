...
class Antispam {
	...
	*/
	...
	var $img_path 		=   './assets/captcha/'; // Image path
	var $img_width 		= 	170;   	// CODE_WIDTH
	...
	var $char_set 		= 	"abdeghnqrtABCDEFGHJKLMNPQRSTUVWXYZ2345689";	//CODE_ALLOWED_CHARS
	...
	function get_antispam_image( $override = array() ) {
		...
		for ($i = 0; $i < $this->char_length ; $i++) {
			...
			imagettftext($img, $this->font_size, $angle, $x, $y, $color, realpath($font), $char);
		...
		return array('word' => $code, 'time' => $now, 'image' => $img_markup, 'path' => $this->img_url . $img_name );
